...
generate_ssh_key() {
  ...
  sudo su -c "ssh-keygen -t ed25519 -C '$USER_EMAIL'" $USER
  log "SSH key generated for user $USER."

  log "Displaying SSH public key. Please copy it and add it to your GitHub account."
  sudo su -c "cat ~/.ssh/id_ed25519.pub" $USER
...
install_node_pm2() {
  log "Installing Node.js and PM2 for user $USER."
  sudo su -c "curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.1/install.sh | bash" $USER
  sudo su -c "source ~/.bashrc && nvm install --lts && nvm use --lts" $USER
  sudo apt-get install -y npm >/dev/null 2>&1
  sudo su -c "npm install -g pm2" $USER
  log "Node.js and PM2 installation completed for user $USER."
}
...
deploy_backend_application() {
  ...
  sudo su -c "mkdir -p $USER_DIR && cd $USER_DIR && git clone git@github.com:$PROJECT_NAME/$BACKEND_APPLICATION_NAME.git $BACKEND_APPLICATION_NAME" $USER
  sudo su -c "cd $USER_DIR/$BACKEND_APPLICATION_NAME && npm install && pm2 start --name '$BACKEND_APPLICATION_SERVICE_NAME' npm -- run start:dev" $USER
...
deploy_frontend_application() {
  ...
  sudo su -c "cd $USER_DIR && git clone git@github.com:$PROJECT_NAME/$FRONTEND_APPLICATION_NAME.git $FRONTEND_APPLICATION_NAME" $USER
  sudo su -c "cd $USER_DIR/$FRONTEND_APPLICATION_NAME && npm install && pm2 start --name '$FRONTEND_APPLICATION_SERVICE_NAME' npm -- run dev" $USER
